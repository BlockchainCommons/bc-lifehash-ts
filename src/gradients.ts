...
type Family = (entropy: BitEnumerator, hueGenerator: ColorFunc) => ColorFunc;
type FiducialFamily = (entropy: BitEnumerator) => ColorFunc;

/** The four gradient families, indexed by two bits of entropy. */
const FAMILIES: readonly Family[] = [monochromatic, complementary, triadic, analogous];
const FIDUCIAL_FAMILIES: readonly FiducialFamily[] = [
  monochromaticFiducial,
  complementaryFiducial,
  triadicFiducial,
  analogousFiducial,
];

/** The gradient for a version, drawn from the digest's bits. */
export function selectGradient(entropy: BitEnumerator, version: LifeHashVersion): ColorFunc {
  ...
  const family = entropy.nextUint2();

  if (version === "fiducial") {
    return FIDUCIAL_FAMILIES[family](entropy);
  }

  // version1 keeps its original gamut: pure HSB hues for the monochromatic
  // family and the wide spectrum for the others.
  const hueGenerator =
    version === "version1" ? (family === 0 ? makeHue : spectrum()) : spectrumCmykSafe();
  return FAMILIES[family](entropy, hueGenerator);
}
