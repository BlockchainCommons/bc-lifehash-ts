 ...
 *
 * Usage:
 *   bun scripts/api-report.ts --local   # (re)generate api/lifehash.api.md
 ...
 * (api/lifehash.api.md) is the reviewable record of the public surface: every
...
try {
  ...
  if (result.apiReportChanged && !local) {
    console.error(
      "Public API surface changed but api/lifehash.api.md was not updated.\n" +
  ...
  console.log(
    local
      ? "API report (api/lifehash.api.md) is up to date."
